...
resource "aws_subnet" "public_subnet_az_a" {
  ...
  tags = merge(
    {
      "Name" = format(
        "${var.environment_name}-${var.public_subnet_interfix}-a-${count.index+1}",
      )
    },
    var.additional_tags
  )
...
resource "aws_subnet" "public_subnet_az_b" {
  ...
  tags = merge(
    {
      "Name" = format(
        "${var.environment_name}-${var.public_subnet_interfix}-b-${count.index+1}",
      )
    },
    var.additional_tags
  )
...
resource "aws_subnet" "public_subnet_az_c" {
  ...
  tags = merge(
    {
      "Name" = format(
        "${var.environment_name}-${var.public_subnet_interfix}-c-${count.index+1}",
      )
    },
    var.additional_tags
  )
